		...
		$columns = array(
			'username' => array(
				...
				"calculate" => function ($label, $user) {
					...
					// debug($user);
					$color = "black";
					if ( isset( $user['class'] ) && isset( $wow_class_colors[ $user['class'] ] ) ) {
						$color = $wow_class_colors[ $user['class'] ];
					}

					// return $label;
					return '<a style="color: ' . $color . '" href="/admin/users/edit/' . $user['id'] . '">' . $user['username'] . '</a>';
				}
			),
			'email' => array(
				"label" => "Email",
