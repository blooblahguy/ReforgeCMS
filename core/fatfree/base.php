...
final class Base extends Prefab implements ArrayAccess {
	...
	function lexicon( $path, $ttl = 0 ) {
		...
		if ( $ttl && $cache->exists(
			...
			return $lex;
		$lex = [];
		foreach ( $languages as $lang ) foreach ( $this->split( $path ) as $dir )
	...
	function blacklisted( $ip ) {
		if ( $this->hive['DNSBL'] &&
			...
			foreach ( is_array( $this->hive['DNSBL'] ) ? $this->hive['DNSBL'] :
	...
	function __construct() {
		...
		$check = false; //error_reporting( ( E_ALL | E_STRICT ) & ~( E_NOTICE | E_USER_NOTICE ) );
